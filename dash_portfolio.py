        ...
        dbc.Card(
            dbc.CardBody([
                dbc.Row([
                    ...
                    dbc.Col([dbc.Button("Update", id="button-update", className="me-2", n_clicks=0)], style={"width": "10%"}),
                ...
                html.Br(),
                dbc.Row([
                    dbc.Col([dcc.Graph(id='fig_pf_instr_adj_close', figure=get_fig_pf_instr_adj_close(account))], style={"width": "15%"}),
...
def get_fig_perf() -> go.Figure:
    ...
    return go.Figure(data=[go.Table(
        ...
        cells=dict(
            values=build_content_portfolio.pf_data.perf_dct[OutDataTabs.RISK_METRICS].dropna().round(3).reset_index().T.values.tolist(),
...
def get_fig_monthly_ret() -> go.Figure:
    return go.Figure(data=[go.Histogram(x=build_content_portfolio.pf_data.perf_dct[OutDataTabs.RETURNS_MONTHLY]['return'].values,
...
def get_fig_pf_instr_adj_close(account: Accounts) -> go.Figure:
