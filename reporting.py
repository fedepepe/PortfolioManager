...
def compute_portfolio_metrics(nav: Optional[pd.Series] = None,
    ...
    downside_volatility_hist = math.sqrt(ANN_FACTOR_DICT[freq]) * returns_resampled.rolling(len(nav),
                                                                                            min_periods=2).apply(
        compute_downside_volatility)
    sortino_ratio_hist = ((pa_return_hist - risk_free_rate) / downside_volatility_hist).rename('Sortino ratio')
